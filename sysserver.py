...
def _sampler(stop_event: threading.Event, interval: float = 2.0) -> None:
    """Sampler thread that measures CPU and memory periodically.

    Use a blocking psutil.cpu_percent(interval=interval) call so the
    measurement is taken over the sampling window. This is more reliable
    than repeated non-blocking calls which can return 0.0 on some systems.
    """
    while not stop_event.is_set():
        # This blocks for `interval` seconds while measuring CPU usage.
...
if __name__ == "__main__":
    try:
        ...
        # Default to port 8001 to avoid conflicting with the weather server (8000).
        host = os.getenv("SYS_MCP_HOST", "127.0.0.1")
        port = int(os.getenv("SYS_MCP_PORT", "8001"))
